...
d_colNames = tibble('extracted_col_names' = colnames(data)) # All of these names have an X as a prefix
temp = separate(d_colNames, extracted_col_names, into = c('field_id', 'instance_id', 'coding_id'), sep = "\\.") # Creat temp object with the output of the extracted col names split into three cols
d_colNames = as_tibble(cbind(d_colNames, temp)) # bind to create the composite table
d_colNames = mutate(d_colNames, field_id = field_id %-% 'X') #remove the 'X' from the field_id column


# Alter data_dict Field_ID to be a character string so that can left_join()
data_dict$Field_ID = as.character(data_dict$Field_ID)
...
d_colNames$Description = gsub(" ", "_", d_colNames$Description)
