...
results_x <- results %>%
  mutate(
    or_ci = paste(odds_ratio, " [", lower_bd, ", ", upper_bd, "]", sep = ""),
    pval = ifelse(pval == 0, "<0.001", pval)
  ) %>%
  select(indep_variable, category, or_ci, pval)

library(xtable)
results_table <- xtable(results_x, row.names = FALSE)
?xtable
print(results_table, type = "Latex")
write.csv(results, "univar_results.csv", row.names = FALSE)
